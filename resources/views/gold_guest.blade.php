...
<div class="container py-5">
    ...
    @if($goldPrices && $goldPrices['ornament_sell'] != 0 && $goldPrices['ornament_buy'] != 0)
        <table class="gold-table">
            ...
            <tbody>
                <tr>
                    ...
                    <td>{{ $goldPrices['ornament_buy_gram'] ?? 'n/a' }}</td>
                </tr>
            </tbody>
        </table>
    @else
    ...
    <form id="goldForm"
        method="POST"
        action="{{ auth()->check() ? route('gold.request.store') : route('gold.submit_guest') }}"
        @if(!$goldPrices || $goldPrices['ornament_sell'] == 0 || $goldPrices['ornament_buy'] == 0)
        ...
        <button type="button" class="btn btn-primary" id="showModalBtn"
            @if(!$goldPrices || $goldPrices['ornament_sell'] == 0 || $goldPrices['ornament_buy'] == 0) disabled @endif>
            ส่งคำขอผ่อนทอง
        </button>
    </form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const goldPrice = parseFloat('{{ isset($goldPrices["ornament_sell"]) ? str_replace(",", "", $goldPrices["ornament_sell"]) : 0 }}');
